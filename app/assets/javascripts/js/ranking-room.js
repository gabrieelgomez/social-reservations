...
$('.js-hotelRoom').not('.disabled-content').on('click', function () {
  ...
  var filtered_json = find_in_object(JSON.parse(rankings_json), {ranking_id: ranking_id_radio});
  if ((filtered_json[6]['price_cop'] == 0) && (kids != 0)){
    ...
    if ($(this).is(':checked')) {
      $.each(filtered_json, function(i, item) {
        let json_parse = {ranking_id: ranking_id_radio, type:item.type_room}
...
$('.js-typeRoom').on('click', function () {
  ranking_id_radio = $(this).val();
  json_parse = JSON.parse(ranking_id_radio);
  that = $(this);
  // Passing JSON methods
    var filtered_json = find_in_object(JSON.parse(rankings_json), {ranking_id: json_parse['ranking_id']});
    $.each(filtered_json, function(i, item) {
      if (item.price_cop > 0 && item.type_room == json_parse['type']) {
        if (that.is(':checked')) {
          ...
          $(`input[name='square_circuit[][${this.type_room}]']`).val('')
